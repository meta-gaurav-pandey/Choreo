...
public class LinkedInMapper extends AbstractMediator {
	...
	public boolean mediate(MessageContext context) {
		...
		if (source.equals("linkedin")) {
			...
			if(isFull.equals("No")) {
			context.setProperty("parentData", parentNameData(jsonPayloadToString));
			}
	...
	public JSONObject getJSONObject(String key, String value) {
		System.out.println("Map linkedin JSon");
	...
	public String getLinkedinParentMappingJson(String jsonPayloadToString) {
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			if (!jsonObject.getString("parent_name").equals("")) {

				jsonObject.put("source", "Linkedin-parent");
				jsonObject.put("Comp_Name", checkKey("social_url", jsonObject));
				jsonObject.put("parentName", jsonObject.getString("parent_name"));
	...
	public JSONArray parentNameData(String InputjsonObject) {
